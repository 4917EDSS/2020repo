...
constexpr int kClimbSpoolBtn=4;
...
void RobotContainer::configureButtonBindings() {
  ...
  frc2::JoystickButton m_shooterBtn(&m_driverController, kShooterBtn);
  m_shooterBtn.WhenPressed(ShootCmd(&m_shooterSub, &m_intakeSub, 3000));

  frc2::JoystickButton m_intakeBtn(&m_operatorController, kIntakeBtn);
  m_intakeBtn.WhenHeld(IntakeCmd(&m_intakeSub));

  frc2::JoystickButton m_climbReleaseBtn(&m_operatorController, kClimbReleaseBtn);
  m_climbReleaseBtn.WhenPressed(ClimbReleaseCmd(&m_climberSub));

  frc2::JoystickButton m_climbSpoolBtn(&m_operatorController, kClimbSpoolBtn);
